/*
    -- MAGMA (version 2.6.2) --
       ...
       @generated from src/zgeqrf_gpu.cpp, normal z -> c, Wed Apr 20 17:36:16 2022
*/
    ...
    magma_int_t *info )
{
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= minmn) {
        /* Use CPU code. */
        if ( MAGMA_SUCCESS != magma_cmalloc_cpu( &work, 2*m*n )) {
            *info = MAGMA_ERR_HOST_ALLOC;
            return *info;
        }
        magma_cgetmatrix(m, n, dA, ldda, work, m, NULL );                                                              
        lhwork = m*n;
        lapackf77_cgeqrf( &m, &n, work, &m, tau, work+m*n, &lhwork, info );
        magma_csetmatrix( m, n, work, m, dA, ldda, NULL );
