/*
    -- MAGMA (version 2.6.2) --
       ...
       @generated from src/zpotrf_gpu.cpp, normal z -> c, Wed Apr 20 17:36:09 2022
*/
    ...
    magma_int_t nb, magma_mode_t mode )
{
    ...
    *info = 0;
    ...
    if (mode == MagmaHybrid) {
        if (nb <= 1 || 4*nb >= n ) {
            /* Use CPU code. */
            if ( MAGMA_SUCCESS != magma_cmalloc_cpu( &work, n*n )) {
                *info = MAGMA_ERR_HOST_ALLOC;
                return *info;
            }
            magma_cgetmatrix( n, n, dA(0,0), ldda, work, n, NULL);
            lapackf77_cpotrf(lapack_uplo_const(uplo), &n, work, &n, info );
            magma_csetmatrix( n, n, work, n, dA(0,0), ldda, NULL);
            magma_free_cpu( work );  work=NULL;

            return *info;
        }
        else if (MAGMA_SUCCESS != magma_cmalloc_pinned( &work, nb*nb )) {
            *info = MAGMA_ERR_HOST_ALLOC;
            goto cleanup;
        }
    }
